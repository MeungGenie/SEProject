...
public class Board extends JFrame {
	...
	private static final int INTERVAL_ANI = 50;
...
//	}
	...
	void lineRemove() {
		...
		for (int i = 0; i < line.size(); i++) {
			System.out.println("타이머00");
			lineRemoveDelay(line.get(i));
		}

		while (iter.hasNext()) {
			removeT = new Timer(600, e -> {
				int index = 0;
				
				System.out.println("타이머11");
				index = iter.next();
			...
			removeT.setRepeats(false);
			removeT.start();
		}
		

		/*
		 * for (int i = 0; iter.hasNext(); i++) { index = iter.next();
		 * lineRemoveDelay(line.get(i)); for(int j = index; j > 1; j--) { for(int k = 0;
		 * k < WIDTH; k++) { board[j][k] = board[k-1][k]; } } index = 0; eraseCnt++;
		 * getScore(eraseCnt, "line"); setScore(); }
		 */
	}

	private void lineRemoveDelay(int lineNum) {
		Timer aniTimer;
		int aniDelay = INTERVAL_ANI;

		for (int cnt = 0; cnt < 6; cnt++) {
			if (cnt % 2 == 0) {
				System.out.println("타이머22");
				aniTimer = new Timer(cnt * aniDelay, e -> removedLinePaint(lineNum, Color.ORANGE));
			} else {
				System.out.println("타이머33");
				aniTimer = new Timer(cnt * aniDelay, e -> removedLinePaint(lineNum, Color.BLACK));
			}
	...
	private void removedLinePaint(int lineNum, Color color) {
		System.out.println("타이머44");
		SimpleAttributeSet removedLineDoc = new SimpleAttributeSet();
		StyleConstants.setForeground(removedLineDoc, color);

		boardDoc.setCharacterAttributes((lineNum + 1) * (WIDTH + 3) + 1, WIDTH, removedLineDoc, true);

		StyleConstants.setFontSize(removedLineDoc, 20);
		...
		// boardDoc.setCharacterAttributes((lineNum + 1) * (WIDTH+3)+1, WIDTH,
		// removedLineDoc, true);
	...
	protected void moveRight() {
		eraseCurr();
		if (x < WIDTH - curr.width() && collisionRight() == false)
			x++;
		placeBlock();
	}
	...
	public int getInterval(int blockNumber, int eraseCnt) {
		...
		System.out.println("Created : " + blockNumber + "   Removed : " + eraseCnt + "   intervalByMode"
				+ intervalByMode + "   interval Number : " + setting.intervalNumber);
		...
		public void keyPressed(KeyEvent e) {
			if (SettingValues.getInstance().keyChoose == 1) {
				...
				case KeyEvent.VK_ESCAPE:
					...
					int choice = JOptionPane.showOptionDialog(null, "What Do You Want?", "Stop", 0, 0, null, stopOption,
							stopOption[1]);
					switch (choice) {
					case 0:
						int confirm1 = JOptionPane.showConfirmDialog(null, "Are you sure?", "Confirm",
					...
					case 1:
						...
						break;
					case 2:
						int confirm2 = JOptionPane.showConfirmDialog(null, "Are you sure?", "Confirm",
								JOptionPane.YES_NO_OPTION);
						if (confirm2 == 0) {
