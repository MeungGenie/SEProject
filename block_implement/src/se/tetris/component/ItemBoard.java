...
public class ItemBoard extends JFrame {
	...
	public static final char BORDER_CHAR = 'H';
	...
	*/
	...
	protected void blockRotate() {
		eraseCurr();
		if (collisionRight() == false && collisionLeft() == false && collisionBottom() == false) {			
			curr.rotate();
		      if (rtRotateColl())
		    	  x = 10 - curr.width();
		      if (btRotateColl())
